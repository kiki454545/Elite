...
async function importAds() {
  try {
    console.log('📥 Importation des annonces de Msida...\n')
    ...
    for (let i = 0; i < jsonData.ads.length; i++) {
      ...
      try {
        ...
        // 1. Créer un profil utilisateur avec email standardisé
        const email = `escortemalte${i + 15}@gmail.com` // Commence à 15 car San Gwann va de 1 à 14
        ...
        // 2. Attendre que le profil soit créé par le trigger (délai de 2 secondes)
        await new Promise(resolve => setTimeout(resolve, 2000))

        // 3. Vérifier si le profil existe, sinon le créer
        ...
        if (!existingProfile) {
          ...
          // Créer le profil manuellement avec seulement les champs de base
          const profileData = {
            id: userId,
            email: email,
            username: ad.name,
...
console.log('🚀 Démarrage de l\'importation des annonces de Msida...\n')
importAds()
