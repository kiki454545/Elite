...
import { Heart, MapPin, Eye, ChevronLeft, ChevronRight, Loader2, X, Check, MessageCircle, Crown } from 'lucide-react'
...
export function PremiumGrid() {
  ...
  // États pour les annonces premium
  ...
  // Charger les annonces premium (filtrées par pays si nécessaire)
  useEffect(() => {
    async function loadPremiumAds() {
      try {
        ...
        const premiumAdsData: Ad[] = ads
          .filter((item: any) => premiumProfilesMap.has(item.user_id))
          .map((item: any) => {
            const profile = premiumProfilesMap.get(item.user_id)
            return {
              id: item.id,
              userId: item.user_id,
    ...
    loadPremiumAds()
  }, [selectedCountry]) // Recharger quand le pays change
