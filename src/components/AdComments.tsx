...
export function AdComments({ adId, adOwnerId }: AdCommentsProps) {
  ...
  function formatDate(dateString: string) {
    ...
    if (diffHours < 24) return `Aujourd'hui ${hours}:${minutes}`
    if (diffDays === 1) return `Hier ${hours}:${minutes}`
    return fullDateTime
  }
              ...
              >
                {loading ? (
                  ...
                  <>
                    ...
                    Voir plus de commentaires ({totalCount - comments.length} restants)
                  </>
                )}
              </button>
            </div>
      ...
      <AnimatePresence>
        {showSuccessMessage && (
          ...
          >
            ...
            <span className="font-medium">Commentaire signalé avec succès !</span>
          </motion.div>
        )}
      </AnimatePresence>
